...
# Build MZ2SYNTH Fortran code to WebAssembly
# Strategy: Fortran → C (via f2c) → WASM (via emcc)

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORKSPACE_DIR="$(dirname "$SCRIPT_DIR")"
FORTRAN_SOURCE_DIR="$WORKSPACE_DIR/fortran-source"
BUILD_DIR="$WORKSPACE_DIR/libs/mz2synth-wasm"
WASM_OUTPUT_DIR="$BUILD_DIR/public"

echo "🌐 Building MZ2SYNTH for WebAssembly..."

# Check if fortran source exists
if [ ! -d "$FORTRAN_SOURCE_DIR" ]; then
    echo "❌ Fortran source not found. Run 'npm run fortran:update' first."
    exit 1
fi
...
if [ -f "$HOME/emsdk/emsdk_env.sh" ]; then
    ...
    echo "❌ Emscripten not found. Run 'npm run wasm:setup' first."
    exit 1
fi
...
echo "📝 Step 1: Creating C implementation of MZ2SYNTH..."
echo "   Note: Using direct C implementation (f2c not needed for Fortran 2003)"

echo "🔧 Step 2: Generating WASM wrapper..."

# Create a C wrapper for the WASM module
cat > "$BUILD_DIR/src/mz2_wasm.c" << 'EOF'
/*
...
echo "🔨 Step 3: Compiling to WebAssembly..."

# Compile C to WASM with Emscripten
emcc "$BUILD_DIR/src/mz2_wasm.c" \
    -o "$WASM_OUTPUT_DIR/mz2synth.js" \
    -s WASM=1 \
    -s EXPORTED_RUNTIME_METHODS='["ccall","cwrap","setValue","getValue","UTF8ToString","stringToUTF8"]' \
    -s EXPORTED_FUNCTIONS='["_malloc","_free","_init_oscillators","_load_ppm_data","_synthesize_audio","_get_audio_sample_count","_cleanup"]' \
    -s ALLOW_MEMORY_GROWTH=1 \
    -s MODULARIZE=1 \
    -s EXPORT_NAME="createMZ2SynthModule" \
...
echo "  - $WASM_OUTPUT_DIR/mz2synth.js"
echo "  - $WASM_OUTPUT_DIR/mz2synth.wasm"
echo ""

# Copy to public directory for Vite
cp "$WASM_OUTPUT_DIR/mz2synth.js" "$WORKSPACE_DIR/public/"
cp "$WASM_OUTPUT_DIR/mz2synth.wasm" "$WORKSPACE_DIR/public/"
